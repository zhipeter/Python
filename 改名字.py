    ...
    for name in files:
        ...
        if dirname == "动漫":
            ...
            os.rename(
                ...
                os.path.join(root, "Anime" + str(int[0]) + fext))
        elif dirname == "风景":
            int[1] += 1
            os.rename(
                os.path.join(root, name),
                os.path.join(root, "Screen" + str(int[1]) + fext))
        elif dirname == "宽屏":
            int[2] += 1
            os.rename(
                os.path.join(root, name),
                os.path.join(root, "WidthScreen" + str(int[2]) + fext))
        elif dirname == "美女":
            int[3] += 1
            os.rename(
                os.path.join(root, name),
                os.path.join(root, "Beauty" + str(int[3]) + fext))
        elif dirname == "游戏":
            int[4] += 1
            os.rename(
                os.path.join(root, name),
